...
export function withAuthMiddleware(middleware: CustomMiddleware): CustomMiddleware {
  return async (request, event, response) => {
    ...
    const normalizedPath = pathName.startsWith(`/${locale}`) ? pathName.replace(`/${locale}`, '') : pathName
    ...
    if (!session && isProtectedRoute) {
      ...
      return Response.redirect(new URL(`/${locale}/sign-in?callbackUrl=${encodedCallbackUrl}`, nextUrl))
    }
